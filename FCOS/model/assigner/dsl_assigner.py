...
from ..iou_loss import bbox_overlaps
...
class DynamicSoftLabelAssigner(BaseAssigner):
    ...
    def __init__(self, topk=25, iou_factor=3.0):
        self.topk = topk
    ...
    def assign(
        ...
        '''
        # center-sampling (added by raymond)
        sample_radiu_ratio = 1.5
        max_stride = strides[len(strides)//2]
        radiu=max_stride*sample_radiu_ratio

        gt_center_x=(gt_bboxes[...,0]+gt_bboxes[...,2])/2
        gt_center_y=(gt_bboxes[...,1]+gt_bboxes[...,3])/2

        center_x1 = gt_center_x - radiu
        center_y1 = gt_center_y - radiu
        center_x2 = gt_center_x + radiu
        center_y2 = gt_center_y + radiu
        center_bboxes = torch.stack([center_x1, center_y1, center_x2, center_y2]).T
        center_bboxes[center_bboxes<0.]=0.

        clt_ = prior_center[:, None] - center_bboxes[:, :2]
        crb_ = center_bboxes[:, 2:] - prior_center[:, None]
        cdeltas = torch.cat([clt_, crb_], dim=-1)
        is_in_center = cdeltas.min(dim=-1).values > 0
        valid_center_mask = is_in_center.sum(dim=1) > 0
        # center-sampling done
        '''
        ...
        #valid_mask = valid_mask & valid_center_mask
        ...
        #self.visualize(orig_img, gt_bboxes, prior_center[valid_mask], center_bboxes)
        matched_pred_ious, matched_gt_inds = self.dynamic_k_matching(
            cost_matrix, pairwise_ious, num_gt, valid_mask
        )

        #self.visualize(orig_img, gt_bboxes, prior_center[valid_mask])
    ...
    #def visualize(self, img, boxes, points, center_bboxes):
    def visualize(self, img, boxes, points):
        ...
        '''
        for i, box in enumerate(center_bboxes):
            pt1 = (int(box[0]), int(box[1]))
            pt2 = (int(box[2]), int(box[3]))
        ...
        for p in points:
            ...
            img = cv2.circle(img, (px, py), 2, (0, 0, 255), 1) 
        
        #img = cv2.resize(img, (640, 640), interpolation=cv2.INTER_LINEAR)
        #cv2.imshow('valid points', img)
        #cv2.waitKey(0)
